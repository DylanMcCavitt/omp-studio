    ...
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center justify-between gap-4 border-b border-border px-6 py-4">
        <div className="min-w-0">
          ...
          <p className="truncate text-sm text-ink-muted" title={enabled && cwd ? cwd : undefined}>
            {enabled && cwd
      ...
      {createError && (
        ...
        >
          <span className="min-w-0 flex-1 break-words [overflow-wrap:anywhere]">Terminal failed to start: {createError}</span>
          <Button
            size="sm"
            variant="subtle"
            onClick={() => void spawnTerminal()}
      ...
      {externalError && (
        <div
          ...
          className="flex shrink-0 items-center justify-between gap-3 border-b border-danger/30 bg-danger/5 px-4 py-2 text-sm text-danger"
        >
          <span className="min-w-0 flex-1 break-words [overflow-wrap:anywhere]">External terminal failed to open: {externalError}</span>
          <Button
            size="sm"
            variant="subtle"
            onClick={() => void openExternalTerminal()}
